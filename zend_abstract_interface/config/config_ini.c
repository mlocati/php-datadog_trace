...
int16_t zai_config_initialize_ini_value(zend_ini_entry **entries, int16_t ini_count, zai_string_view *buf,
    ...
    if (is_fpm) {
        for (int16_t i = 0; i < ini_count; ++i) {
            ...
            if (ZSTR_LEN(ini_str) != default_value.len || strcmp(ZSTR_VAL(ini_str), default_value.ptr) != 0) {
                ...
                zval new_zv;
                ZVAL_UNDEF(&new_zv);
                if (!zai_config_decode_value((zai_string_view)ZAI_STRING_FROM_ZSTR(ini_str), memoized->type, memoized->parser, &new_zv, true)) {
                    continue;
                }
    ...
    for (int16_t i = 0; i < ini_count; ++i) {
        ...
        if (inizv != NULL && !parsed_ini_value) {
            ...
            zval new_zv;
            ZVAL_UNDEF(&new_zv);
            if (!zai_config_decode_value((zai_string_view)ZAI_STRING_FROM_ZSTR(Z_STR_P(inizv)), memoized->type, memoized->parser, &new_zv, true)) {
                continue;
            }
            zai_config_dtor_pzval(&new_zv);
