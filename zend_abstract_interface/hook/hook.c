...
static zend_long zai_hook_request_install(zai_hook_t *hook) {
    ...
    zai_string_view scope = hook->scope ? (zai_string_view)ZAI_STRING_FROM_ZSTR(hook->scope) : (zai_string_view)ZAI_STRING_EMPTY;
    zend_function *function = zai_hook_lookup_function(scope, (zai_string_view)ZAI_STRING_FROM_ZSTR(hook->function), &ce);
    if (function) {
        ...
        if (!--hook->refcount) {
            ...
            if ((hook_zv = zend_hash_index_find(&hooks->hooks, (zend_ulong) -hook->id))) {
                if (Z_TYPE_INFO_P(hook_zv) == ZAI_IS_SHARED_HOOK_PTR) {
                    ...
                    zend_class_entry *ce = NULL;
                    zend_function *origin_func = zai_hook_lookup_function((zai_string_view)ZAI_STRING_FROM_ZSTR(hook->scope), (zai_string_view)ZAI_STRING_FROM_ZSTR(hook->function), &ce);
