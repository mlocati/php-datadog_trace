...
ADD https://aka.ms/vs/14/release/vs_buildtools.exe /tmp/vs_buildtools.exe
...
RUN powershell.exe Expand-Archive /tmp/vim90w32.zip
RUN move C:\tmp\vim90w32\vim\vim90\tee.exe C:\Windows\
RUN move C:\tmp\vim90w32\vim\vim90\vim.exe C:\Windows\
RUN move C:\tmp\vim90w32\vim\vim90\xxd.exe C:\Windows\
...
WORKDIR /php-sdk
RUN git checkout php-sdk-2.1.10
