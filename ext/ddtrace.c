...
_Atomic(int) ddtrace_warn_legacy_api;
...
#ifndef _WIN32
        if (get_global_DD_TRACE_TELEMETRY_ENABLED() || get_global_DD_TRACE_SIDECAR_TRACE_SENDER())
#endif
        {
...
static void ddtrace_activate(void) {
    ...
    if (!DDTRACE_G(disable) && (get_global_DD_TRACE_TELEMETRY_ENABLED() || get_global_DD_TRACE_SIDECAR_TRACE_SENDER())) {
...
_Atomic(int) ddtrace_warn_span_id_legacy_api = 1;
...
PHP_FUNCTION(DDTrace_close_span) {
    ...
    uint64_t finish_time = (uint64_t)(finish_time_seconds * ZEND_NANO_IN_SEC);
...
void ddtrace_read_distributed_tracing_ids(bool (*read_header)(zai_string_view, const char *, zend_string **header_value, void *data), void *data) {
    ...
    if (parse_b3_single && read_header((zai_string_view)ZAI_STRL_VIEW("B3"), "b3", &b3_header_str, data)) {
    ...
    if (parse_datadog_meta_headers) {
        read_header((zai_string_view)ZAI_STRL_VIEW("X_DATADOG_ORIGIN"), "x-datadog-origin", &DDTRACE_G(dd_origin), data);
    }

    if (parse_datadog && read_header((zai_string_view)ZAI_STRL_VIEW("X_DATADOG_TRACE_ID"), "x-datadog-trace-id", &trace_id_str, data)) {
        zval trace_zv;
    ...
    } else if (parse_b3 && read_header((zai_string_view)ZAI_STRL_VIEW("X_B3_TRACEID"), "x-b3-traceid", &trace_id_str, data)) {
        DDTRACE_G(distributed_trace_id) = dd_parse_b3_trace_id(ZSTR_VAL(trace_id_str), ZSTR_LEN(trace_id_str));
        zend_string_release(trace_id_str);
    }

    if (DDTRACE_G(distributed_trace_id).low || DDTRACE_G(distributed_trace_id).high) {
        if (parse_datadog && read_header((zai_string_view)ZAI_STRL_VIEW("X_DATADOG_PARENT_ID"), "x-datadog-parent-id", &parent_id_str, data)) {
            zval parent_zv;
            ZVAL_STR(&parent_zv, parent_id_str);
            DDTRACE_G(distributed_parent_trace_id) = ddtrace_parse_userland_span_id(&parent_zv);
            zend_string_release(parent_id_str);
        } else if (parse_b3 && read_header((zai_string_view)ZAI_STRL_VIEW("X_B3_SPANID"), "x-b3-spanid", &parent_id_str, data)) {
            zval parent_zv;
            ZVAL_STR(&parent_zv, parent_id_str);
    ...
    if (parse_datadog && read_header((zai_string_view)ZAI_STRL_VIEW("X_DATADOG_SAMPLING_PRIORITY"), "x-datadog-sampling-priority", &priority_str, data)) {
        priority_sampling = strtol(ZSTR_VAL(priority_str), NULL, 10);
        zend_string_release(priority_str);
    } else if (parse_b3 && read_header((zai_string_view)ZAI_STRL_VIEW("X_B3_SAMPLED"), "x-b3-sampled", &priority_str, data)) {
        ...
        zend_string_release(priority_str);
    } else if (parse_b3 && read_header((zai_string_view)ZAI_STRL_VIEW("X_B3_FLAGS"), "x-b3-flags", &priority_str, data)) {
    ...
    if (parse_datadog_meta_headers && read_header((zai_string_view)ZAI_STRL_VIEW("X_DATADOG_TAGS"), "x-datadog-tags", &propagated_tags, data)) {
    ...
    if (parse_tracestate && read_header((zai_string_view)ZAI_STRL_VIEW("TRACEPARENT"), "traceparent", &traceparent, data)) {
        do {
            // skip whitespace
       ...
       // header format: "[*,]dd=s:1;o:rum;t.dm:-4;t.usr.id:12345[,*]"
        if (parse_tracestate && read_header((zai_string_view)ZAI_STRL_VIEW("TRACESTATE"), "tracestate", &tracestate, data)) {
