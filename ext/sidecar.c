...
static bool dd_sidecar_connection_init(void) {
    if (!ddtrace_ffi_try("Failed connecting to the sidecar", ddog_sidecar_connect_php(&ddtrace_sidecar, get_global_DD_TRACE_TELEMETRY_ENABLED()))) {
    ...
    if (!ddtrace_sidecar_instance_id) {
        ...
        if (get_global_DD_TRACE_TELEMETRY_ENABLED()) {
