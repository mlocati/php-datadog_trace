...
zend_always_inline zai_string_view dd_from_ddog_char_slice(ddog_CharSlice slice) {
    return (zai_string_view){ .ptr = slice.ptr, .len = slice.len };
}
...
int64_t dd_set_span_probe(const struct ddog_ProbeTarget *target) {
    ...
    zend_long id = zai_hook_install(
            def->scope ? ZAI_STRING_FROM_ZSTR(def->scope) : ZAI_STRING_EMPTY,
            def->function ? ZAI_STRING_FROM_ZSTR(def->function) : ZAI_STRING_EMPTY,
            dd_span_probe_begin,
            dd_span_probe_end,
...
void ddtrace_minit_remote_config(void) {
    ddog_init_live_debugger(&(ddog_LiveDebuggerSetup) {
